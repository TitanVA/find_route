...
class AllTestCase(TestCase):
    ...
    def setUp(self):
        ...
        lst = [
            ...
            Train(name="t5", from_city=self.city_B, to_city=self.city_E, travel_time=5),
    ...
    def test_model_train_train_duplicate(self):
        ...
        try:
            ...
            self.assertEqual({"__all__": ["Запись с такими городами и временем уже существует"]}, e.message_dict)
            self.assertIn("Запись с такими городами и временем уже существует", e.messages)
